...
export class RightClickAnnotator extends Extension {
	...
	onMouseUp(event) {
		...
		if (start.square && endSquare && start.square !== endSquare) {
			// toggle arrow
			const existing = this.chessboard.getArrows(
				arrowType,
				start.square,
				endSquare,
			);
			...
			const existingMarkers = this.chessboard.getMarkers(
				circleType,
				start.square,
			);
		...
		switch (colorKey) {
			...
			case "warning":
				return {
					...
					circleType: MARKER_TYPE.warning,
				};
			default:
				return {
					arrowType: ARROW_TYPE.success,
					circleType: MARKER_TYPE.success,
				};
		}
	}
}
